...
import { Container, Statistic, Segment, Table, Label, Icon, Button, Header, Grid, Dimmer, Loader, Card } from 'semantic-ui-react'
...
class Dashboard  extends Component {
    ...
    componentWillMount = () => {
		...
		.then(data => {
            ...
            if(data.isSuccess) {
                ...
                if(data.customerDetails!==undefined)    // result set is not empty
                    this.setState({
    ...
    outStockCellOnClick = (idx) => {
        ...
        fetch(`/get_asset_type_customer_name?asset_type_id=${this.state.allAssetTypes[idx].id}`, 
        { method: 'GET' })
		.then(result => result.json()) 
		.then(data => {
            if(data.isSuccess) {
    ...
    inStockCellOnClick = (idx) => {
        ...
        fetch(`/in_damaged_stock?id=${this.state.allAssetTypes[idx].id}&status=1`, 
        { method: 'GET' })
		.then(result => result.json()) 
		.then(data => {
    ...
    assetTypeOnClick = (selectedAssetTypeName) => {
		...
		fetch(`/get_all_values?type_name=${selectedAssetTypeName}`,
		{ method: 'GET' })
		.then(r => r.json()) 
        .then(data => {
            if(data.isSuccess) {
                ...
                return(
                    ...
                    {/* <Button basic onClick={()=>console.log(this.state) }>State</Button> */}
                        ...
                        <Grid columns={2} >
                            <Grid.Row >
                                <Grid.Column>
                                <div style={{width: "100%",height:"280px",overflowX: "auto",overflowY:"scroll"}}>
                                    {this.renderAssetStatusTable()}
                                    </div>
                                </Grid.Column>
                                <Grid.Column>
                                    <div style={{width: "100%",height:"285px",overflowX: "auto",overflowY:"scroll"}}>
                            ...
                            <Grid.Row >
                                <Grid.Column>
                                <div style={{width: "100%",height:"400px",overflowX: "auto",overflowY:"scroll"}}>
    ...
    renderCustomerDetails() {
        return(
            <Segment>
                <Label as='a' color='olive' ribbon><Icon name="user circle"/>Customer Details</Label>
                        ...
                        {this.state.allCustomers.map(obj=>{
                            return (
                                ...
                                {this.state.selectedCustomerDetails.Customer_Id===obj.Customer_Id ?
                                    <Table.Cell>
                                    <Label ribbon color="olive">{obj.CName}</Label>
                                    </Table.Cell>
                                    :
    ...
    renderSelectedCustomerDetails() {
        ...
        return[
                ...
                <Card.Content>
                    ...
                    <Card.Description><strong>Customer ID:    </strong>  {this.state.selectedCustomerDetails.Customer_Id}</Card.Description>	
                    <Card.Description><strong>Name:           </strong>  {this.state.selectedCustomerDetails.CName}</Card.Description>	
                    <Card.Description><strong>Previous Alias: </strong>  {this.state.selectedCustomerDetails.Previously_Known_As}</Card.Description>
                    <Card.Description><strong>PAN:          </strong>  {this.state.selectedCustomerDetails.Pan_No}</Card.Description>
                </Card.Content>
                
                <Card.Content extra>
                    <Card.Description><strong>Comments:     </strong>  {this.state.selectedCustomerDetails.Comments}</Card.Description>
                    <br />
                    <div className='ui two buttons'>
                        <Button basic color='green' onClick={this.loadCustomerOrderHistory.bind(this, this.state.selectedCustomerDetails.Customer_Id)}>Order History</Button>
                        <Button basic color='red' onClick={() => this.setState({ selectedCustomerDetails: '' })}>Close</Button>
                    </div>
                </Card.Content>
                </Card>
             ]
    ...
    renderAssetStatusTable () {
        ...
        return (
            ...
            <Segment>
                <Label as='a' color='orange' ribbon><Icon name="shopping basket"/>Asset Summery</Label>
                <Table color="orange" celled unstackable definition striped>
					<Table.Header>
						<Table.Row>
							<Table.HeaderCell><Icon name="tv" color="grey"/>  Asset-Type</Table.HeaderCell>
							<Table.HeaderCell><Icon name="download" color="green"/>  In stock</Table.HeaderCell>
							<Table.HeaderCell><Icon name="upload" color="purple"/>  Out of Stock</Table.HeaderCell>
							<Table.HeaderCell><Icon name="ban" color="red"/>  Damaged</Table.HeaderCell>
							<Table.HeaderCell><Icon name="plus" color="orange"/>  Total</Table.HeaderCell>
						</Table.Row>
					</Table.Header>
					<Table.Body>
                        {this.state.allAssetTypes.map( (asset, idx) =>{
                            ...
                            return (
                                ...
                                key={asset.id}
                                style={{ cursor: "pointer" }}>
                                    <Table.Cell><Icon name="triangle right" color="grey"/>{asset.type_name}</Table.Cell>
                                    {(inStock>0) ? 
                                        <Table.Cell selectable
                                        onClick={this.inStockCellOnClick.bind(this, idx, asset)}>
                                        <a>{inStock}</a></Table.Cell> 
                                    ...
                                    {(damaged>0) ? 
                                        <Table.Cell selectable onClick={this.damagedStockCellOnClick.bind(this, idx, asset)}>
                                        <a>{damaged}</a></Table.Cell> 
                    ...
                    <Table.Footer fullWidth>
                      <Table.Row>
                      <Table.HeaderCell><Icon name="instock"/><strong>TOTAL</strong><Icon name="chevron right"/></Table.HeaderCell>
    ...
    renderOutStockAssetTable () {
        ...
        return (
            <Segment>
                <Label as='a' color='violet' ribbon><Icon name="upload"/>Assets Out of Stock</Label>
                ...
                <Button  color="blue" onClick={()=>{this.setState({currentComponent: componentRender.MAIN})}}><Icon name="undo"/>Back</Button>
            </Segment>
        )
    }
    ...
    renderInStockAssetTable () {
        ...
        return (
            <Segment>
                <Label as='a' color='green' ribbon><Icon name="download"/>Assets In Stock</Label>
                        ...
                        {this.state.assetInStockDetails.map(obj=>{
                            ...
                            return (
                                <Table.Row>
                                    ...
                                    ?
                                    <Table.Cell>{moment(obj.warranty_end_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                    :
                                    ...
                                    ?
                                    <Table.Cell>{moment(obj.procurement_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                    :
                                    <Table.Cell></Table.Cell>
                                    }
                                    {moment(obj.purchase_date).format("YYYY-MM-DD HH:mm:ss")!=='Invalid date'
                                    ?
                                    <Table.Cell>{moment(obj.purchase_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                    :
                                    <Table.Cell></Table.Cell>
                                    }
                                    ...
                                    {moment(obj.transfer_order_date).format("YYYY-MM-DD HH:mm:ss")!=='Invalid date'
                                    ?
                                    <Table.Cell>{moment(obj.transfer_order_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                    :
                                    <Table.Cell></Table.Cell>
                                    }
                                    </Table.Row>
    ...
    renderDamagedStockAssetTable () {
        ...
        return (
            <Segment>
                <Label as='a' color='red' ribbon><Icon name="ban"/>Damaged assets</Label>
    ...
    renderConfigSegment = () => {
        ...
        return (
            ...
            {/* <Segment> */}
                <Label as='a' color='violet' ribbon><Icon name="wrench"/>Config Details</Label>
                <br/><br/>
                <Grid columns={2} >
                    <Grid.Row>
                        <Grid.Column width={4}>
                    ...
                    <Grid.Row>
                        ...
                        {this.state.selectedAssetId && this.state.selectedAssetConfig.length>0 ?
                                ...
                                <Table.Body>
                                    {this.state.selectedAssetConfig.map( (asset) => (
                                        <Table.Row>                               
                                            ...
                                            {moment(asset.create_timestamp).format("YYYY-MM-DD HH:mm:ss")!=='Invalid date'
                                            ?
                                            <Table.Cell>​​{moment(asset.create_timestamp).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                            :
                                            <Table.Cell></Table.Cell>
                                            }
                                            </Table.Row>
                            ...
                            : <Container textAlign="center">
                                {this.state.selectedAssetId && this.state.selectedAssetConfig.length===0 ?
                                    <Header color="grey" size="small">No Config Data available</Header>
                                    : undefined
                                }
                            </Container>
                        }
...
renderCustomerOrderDetailTable = () => {
    ...
    return (
        <div>
            <Label as='a' color='brown' ribbon><Icon name="history"/>Order Details</Label>
                    ...
                    {this.state.selectedCustomerOrderDetails.map(obj=>{
                        ...
                        return (
                            <Table.Row>
                                ...
                                ?
                                <Table.Cell>{moment(obj.rental_begin_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                :
                                <Table.Cell></Table.Cell>
                                }
                                {moment(obj.rental_end_date).format("YYYY-MM-DD HH:mm:ss")!=='Invalid date'
                                ?
                                <Table.Cell>{moment(obj.rental_end_date).format("YYYY-MM-DD HH:mm:ss")}</Table.Cell>
                                :
                                <Table.Cell></Table.Cell>
                                }
                                <Table.Cell>{obj.serial_no}</Table.Cell>
